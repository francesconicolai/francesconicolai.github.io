...
## Working Papers
### [An Impossibility Theorem for Price-Based Risk Constraints](https://francesconicolai.github.io/papers/CCP_impossibility.pdf)
...
<div style="text-align: justify">
Price-based risk constraints map sampled transaction prices into binding margin, haircuts, leverage limits, or target-volatility reallocations. This feedback creates dynamic arbitrage even when the underlying impact model is manipulation-proof in the classical sense. A trader can execute a trigger-and-reverse round trip: trade inside the sampling window to tilt the statistic, force mechanical trading, then unwind into predictable flow. We characterize the optimal attack under both single-asset marking and portfolio marking; under portfolio marking, the problem reduces to a two-portfolio attack strategy. We derive an admissibility test that, given market-impact estimates, yields a liquidity-scaled bound on the maximum size of the constrained sector that regulators can use to limit its scale. Applied to volatility-managed portfolios, realistic market-impact inputs imply that the constrained sector's maximum admissible dollar position in an asset is on the order of 40% of its average daily dollar volume, which is small relative to typical outstanding positions. Risk becomes endogenously manipulable, a related but distinct instability mechanism from leverage-cycle models and the paradox of financial stability.</div>
