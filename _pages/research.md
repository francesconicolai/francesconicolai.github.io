...
<div style="text-align: justify">
Price-based risk constraints can create dynamic arbitrage even when the underlying impact model is manipulation-proof in the classical sense. These constraints map sampled transaction prices into binding margin, haircuts, leverage limits, target-volatility trading rules, or investment mandates. We derive an implementable admissibility test which, given a disclosed rulebook and standard market-impact inputs, (i) screens whether the induced feedback loop admits profitable round trips and (ii) when it does not, outputs a transparent liquidity-scaled capacity bound and vulnerability curves for the constrained sector. The arbitrage channel is a trigger-and-reverse strategy: trade inside the sampling window to tilt the statistic, force mechanical trading, then unwind into predictable flow. We characterize the optimal attack under both single-asset and portfolio rules; under portfolio marking, the problem reduces to a two-portfolio attack strategy. Applied to volatility-managed portfolios, realistic market-impact inputs imply that the constrained sector's maximum admissible dollar position in an asset is on the order of 40% of its average daily dollar volume, which is small relative to typical outstanding positions. Risk becomes endogenously manipulable, especially when measured risk is low; this channel is related to, but distinct from, leverage-cycle models and the paradox of financial stability.</div>
