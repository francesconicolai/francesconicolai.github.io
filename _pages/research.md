...
<div style="text-align: justify">
Price-based risk constraints can generate dynamic arbitrage even when the underlying impact model rules out classical price manipulation. Such constraints map sampled transaction prices into binding requirements: margin, haircuts, leverage limits, volatility-targeting rules, or investment mandates. We derive an admissibility test that uses only the disclosed rulebook and standard market-impact inputs. The test screens whether the induced rule-driven trading admits profitable round trips and, when it does not, delivers a liquidity-scaled capacity bound and vulnerability curves that map sector scale into the share of stress states in which profitable round trips would arise. We characterize optimal attacks under both single-asset and portfolio constraints; with portfolio marking, an optimal strategy trades only two portfolios. 
We apply the test to volatility-managed portfolios and find that admissible sector scale is typically small under realistic impact calibrations: conservative capacity bounds are well below one day of ADV, and vulnerability rises quickly once sector scale reaches one to two days of ADV. More broadly, the paper shows that risk can be endogenously manipulable through the measurement rule itself, with incentives that are strongest when measured volatility is low; this is related to endogenous-risk and leverage-cycle models, but it operates through the mapping from prices to requirements rather than through balance-sheet constraints alone.</div>
